function [bkps,supp]=collectSampleSupport(clusterRanges,SuppReads,SampleInfo,insertSize,readLength)
...
for i=1:length(clusterRanges)
	...
	currBkp.pos1_left=currBkp.pos1_left-3*insertSize;currBkp.pos1_right=currBkp.pos1_right+3*insertSize;
	currBkp.pos2_left=currBkp.pos2_left-3*insertSize;currBkp.pos2_right=currBkp.pos2_right+3*insertSize;
	...
	Supp.pos1=double(Supp.pos1);Supp.pos2=double(Supp.pos2);
	STR_INFO=unique([Supp.str1,Supp.str2],'rows');
	for ki=1:size(STR_INFO,1)
		str1=STR_INFO(ki,1);str2=STR_INFO(ki,2);
		currSupp=Supp(Supp.str1==str1 & Supp.str2==str2,:);	
		if size(currSupp,1)<=2
			continue;
		end	
		split=currSupp(currSupp.split,:);
		nonsplit=currSupp(~currSupp.split,:);			
		...
		if str1==1
			pos1_left=pos1-6*insertSize;pos1_right=pos1+readLength;
		else
			pos1_left=pos1-readLength;pos1_right=pos1+6*insertSize;
		end
		if str2==1
			pos2_left=pos2-6*insertSize;pos2_right=pos2+readLength;
		else
			pos2_left=pos2-readLength;pos2_right=pos2+6*insertSize;
		end
		currRange=dataset(pos1_left,pos1_right,pos2_left,pos2_right,'VarNames',{'pos1_left','pos1_right','pos2_left','pos2_right'});
		currSupp=collectReadsByRange(currSupp,currRange);
		currSupp=currSupp(currSupp.str1==str1 & currSupp.str2==str2,:);	
		if (length(currSupp)<2)
			continue;
		end
		SVidx=SVidx+1;
		%pos1,pos2,currBkp.chr1,currBkp.chr2,str1,str2
		CHR1{SVidx}=currBkp.chr1;CHR2{SVidx}=currBkp.chr2;POS1(SVidx)=pos1;POS2(SVidx)=pos2;STR1(SVidx)=str1;STR2(SVidx)=str2;
		currSupp=sortrows(currSupp,{'SampleGroup','readID'});	
