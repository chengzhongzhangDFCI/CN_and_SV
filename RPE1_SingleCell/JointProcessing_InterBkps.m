...
for i=1:length(CHRs)
	...
	for j=1:length(chrjs)
		...
		cluster_reads=temp(temp.maq1>=30 & temp.maq2>=30,:);	
		...
		tic,	
		fprintf(1,'\n%s <=> %s:',ci,cj);
		curr_cluster=cluster_reads(:,{'chr1','pos1','str1','chr2','pos2','str2'});
		...
		clusters=cluster_pairs(curr_cluster,'JumpSize',insertSize,'Threshold',1,'minCount',minCount,'InterPairs');	
		fprintf(1,'\nTotal: %d clusters\n',length(clusters));
